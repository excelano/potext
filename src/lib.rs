//! Message catalogues in the GNU `.po` format, compiled into a binary as text.
//
// Author: David M. Anderson
// Built with AI assistance (Claude, Anthropic)
//
//! `potext` reads a `.po` catalogue that was `include_str!`ed into the program
//! and looks messages up by the English they were written in. It is the
//! translation layer for the Excelano desktop applications, which are egui and
//! have no toolkit of their own to ask.
//!
//! ```ignore
//! mod i18n {
//!     potext::catalog!();
...
//! ```
//!
//! # Why `.po`, and why not gettext
//!
//! The format was chosen for one property: it can say that a translation has
//! gone *stale*. When an English message is reworded, `msgmerge` pairs the new
//! text with the entry it descended from, carries the old translation over and
//! marks it `#, fuzzy`. This crate refuses to load a fuzzy entry, so the
//! interface falls back to English until somebody has read the new sentence.
//! A key-value catalogue cannot express that: the reworded string is simply a
//! new key, and the old translation sits in the file still looking finished.
//! Over years and several applications, that is the difference that compounds.
//!
//! It is **not** gettext. There is no `.mo`, no text domain, no
//! `bindtextdomain`, and no catalogue on disk to install or find — which is
//! what lets one mechanism behave identically in an App Store sandbox, an MSIX
//! package and a `.deb`, three things that do not agree about where a data file
//! lives. It compiles no C, has no build script, and forbids unsafe in its own
//! source.
//!
//! # One catalogue per crate, and a tag between them
//!
//! [`catalog!`] declares the storage inside the crate that invokes it, so a
//! library and the application using it each hold their own messages. That is
//! deliberate: a single global in this crate would be one catalogue for
//! everybody linked against it, and a published widget's strings have to ship
//! with the widget. The only thing that should cross a crate boundary is a
//! language tag — `widget::set_language("de", ...)` — never a catalogue, so a
//! version skew between the two is harmless.
//!
//! # What a catalogue is worth is what the tooling keeps
//!
//! Nothing here writes or merges a `.po` file. `xgettext` extracts, `msgmerge`
//! merges and marks, `msgfmt` validates, and a pseudolocale catalogue built
//! from the template finds the strings that never reached [`catalog!`] at all.
//! The Excelano repositories carry those as `po/update-po.sh` and
//! `po/pseudo.sh`.
...
impl Catalog {
    /// This message in this catalogue's language, or the English it was written in.
    ///
    /// The English text is the key, so a call site reads as the sentence a
    /// person sees and an untranslated message is not a missing-key
    /// placeholder but the original. The lifetimes tie the answer to the
    /// catalogue, and a catalogue held in a `static` is `'static`, which is
    /// what lets a lookup hand back a `&'static str` rather than a fresh
    /// `String` on every frame of an immediate-mode window.
    #[must_use]
    pub fn t<'a>(&'a self, msgid: &'a str) -> &'a str {
        match self.plain.get(msgid) {
            Some(Message::Single(translated)) => translated,
            // A plural entry reached through `t` is a catalogue that disagrees
            // with its call site. English is the safe answer: it is at least
            // the text the programmer wrote.
    ...
    /// standing for two different things can be two entries. German needs this
    /// more often than English suggests — *open* the verb and *open* the
    /// adjective are `öffnen` and `offen`.
    #[must_use]
    pub fn tc<'a>(&'a self, context: &str, msgid: &'a str) -> &'a str {
    ...
    ///
    /// **The only plural rule understood is two forms chosen by `n != 1`**,
    /// which is what English and German both use and what `msginit` writes for
    /// German. A catalogue declaring anything else keeps its entries but is not
    /// consulted here, and the English forms are used instead — wrong language,
    /// right grammar, which is the better of the two ways to be wrong. A
    /// language with three forms is the day this grows a rule evaluator.
    #[must_use]
    pub fn tn<'a>(&'a self, one: &'a str, other: &'a str, n: u64) -> &'a str {
        let english = if n == 1 { one } else { other };
        if !self.two_forms {
            return english;
        }
        match self.plain.get(one) {
...
/// Declare a catalogue, and the four functions that read it, in this crate.
///
/// Invoke it once, in a module of its own, and use what it declares:
///
/// ```ignore
/// mod i18n {
///     potext::catalog!();
...
/// ```
///
/// It declares `activate`, `set_language`, `t`, `tc` and `tn`. An application
/// calls `activate` once in `main`, handing over each language it ships and
/// that language's `.po` file as text; a library that draws inside somebody
/// else's window takes a tag from its host and calls `set_language` instead,
/// which is why a tag and never a catalogue crosses a crate boundary.
///
/// **The storage it declares belongs to the crate that invoked it**, which is
/// the whole reason this is a macro rather than a global in `potext`. A single
/// global would be one catalogue for every crate linked against this one, and a
/// published widget has to carry its own messages.
///
/// Activation is a plain function call rather than part of the macro, so the
/// list of languages is ordinary Rust: a catalogue can be `#[cfg]`ed out of a
/// release build the way any other expression can.
#[macro_export]
macro_rules! catalog {
    () => {
        /// The catalogue in force in this crate, or nothing, which means English.
        static POTEXT_CATALOG: ::std::sync::OnceLock<$crate::Catalog> =
            ::std::sync::OnceLock::new();

        /// Read the platform's language, pick a catalogue for it, and put it in force.
        ///
        /// Each pair is a language tag and that language's `.po` file as text,
        /// which the caller will have `include_str!`ed. Returns the tag chosen;
        /// `None` means nothing matched and the interface stays in the English
        /// its call sites are written in.
        #[allow(dead_code)]
        pub fn activate(available: &[(&str, &'static str)]) -> Option<String> {
            let preferred = $crate::preferred()?;
            set_language(&preferred, available)
        }

        /// Put a named language in force, whatever the platform says.
        ///
        /// What a library is given by its host, and what an application would
        /// call for a language setting of its own.
        ///
        /// Calling either of these twice is not an error and the second call
        /// does nothing, because a window reads the catalogue on every frame
        /// and swapping it underneath would mean locking on a read that has no
        /// other reason to lock. A language change is a restart, which is what
        /// a desktop application's language setting is everywhere else.
        #[allow(dead_code)]
        pub fn set_language(tag: &str, available: &[(&str, &'static str)]) -> Option<String> {
            let (tag, text) = $crate::choose(available, tag)?;
            let tag = tag.to_owned();
            let _ = POTEXT_CATALOG.set($crate::Catalog::parse(text));
            Some(tag)
        }
        ...
        pub fn tc(context: &'static str, msgid: &'static str) -> &'static str {
            POTEXT_CATALOG
                .get()
                .map_or(msgid, |catalog| catalog.tc(context, msgid))
        }
        ...
        pub fn tn(one: &'static str, other: &'static str, n: u64) -> &'static str {
            POTEXT_CATALOG
                .get()
                .map_or(if n == 1 { one } else { other }, |catalog| {
                    catalog.tn(one, other, n)
                })
        }
    };
}
